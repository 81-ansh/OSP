	...
	if (GameState)
	{
		...
		if (GEngine)
		{
			...
			if (PlayerState)
			{
				...
				GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Cyan, FString::Printf(TEXT("%s has joined the game."), *PlayerName));
			}
		}
	}
}
...
void ALobbyGameMode::Logout(AController* Exiting)
{
	...
	if (PlayerState)
	{
		...
		FString PlayerName = PlayerState->GetPlayerName();
		GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Cyan, FString::Printf(TEXT("%s has exited the game."), *PlayerName));
	}
}
